...
function InsertionSort(values){
   ...
   graphTimeline.push(MakeGraph(values)); //turn them into a graph object

    for(let i = 1; i < values.length; i++){ //for every value

        key = values[i];
        keyIndex = i - 1;
        current = values[keyIndex];
        KeyCurrent(numbers, i);
        while(key < current){

            KeyCurrent(numbers, undefined, keyIndex)
            values[keyIndex + 1] = current;
            Shift(numbers, keyIndex, keyIndex + 1);
            keyIndex--;
            current = values[keyIndex];

        }

        values[keyIndex + 1] = key;
        Insert(values, keyIndex + 1);
...
function draw(){
    ...
    graphTimeline = InsertionSort(numbers); //sort the graph and record the steps taken
