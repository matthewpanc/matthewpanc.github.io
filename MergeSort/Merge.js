...
function DrawGraph(graph){
    ...
    for(let i = 0; i < graph.length; i++){ //for every bar in the graph
        ...
        rect(i * spacing, width - bar.value, spacing, bar.value); //rectangle representing the bar
...
// function LeftAndRight(nums, mid){
...
function MergeSort(arr){
    ...
    if (arr.length > 1){
        ...
        mid = arr.length / 2; //Finding the mid of the array
        let L = arr.slice(0, mid); //Dividing the array elements
        let R = arr.slice(mid, arr.length); //into 2 halves
        ...
        // Take anything left and push it on the end of the array
        while(i < L.length){
            arr[k] = L[i];
            i+=1;
            k+=1;
