...
/**
 * sorts a set of values using a bubble sort algorithm it also keeps track of the steps taken to sort them
 * @param {Number[]} values values to be sorted
 */
function BubbleSort(values){

    //do all the timeline operations in here
    //it will get rid of pointless checks and issues such as needing to store values, things happening out of order, etc .
   ...
   graphTimeline.push(MakeGraph(numbers)); //turn them into a graph object

    while(unsorted){

        unsorted = false;

        for(let i = 0; i < values.length - 1; i++){ //for every value

            key = values[i];

            current = values[i + 1]

            KeyCurrent(values, i, i + 1);

            if(current < key){

                values[i + 1] = key; 
                values[i] = current; //swap the positions of the key and the current index being compared
                unsorted = true; //say that there still is stuff moving arround so do one more loop at least
...
function draw(){
    ...
    graphTimeline = BubbleSort(numbers); //sort the graph and record the steps taken
